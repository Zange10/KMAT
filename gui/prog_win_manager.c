...
GObject *tf_prog_window;
GObject *tf_prog_bar;
GObject *lb_prog_info;


void init_prog_window(GtkBuilder *builder) {
	tf_prog_window = gtk_builder_get_object(builder, "progress_window");
...
static gboolean update_sc_ic_progress_window(gpointer data) {
	if (!gtk_widget_is_visible(GTK_WIDGET(tf_prog_window))) {
