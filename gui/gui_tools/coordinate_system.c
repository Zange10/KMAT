...
void draw_coordinate_system_data(CoordinateSystem *coord_system);
...
void print_data_point_group_points(CSDataPointGroup *group) {
	for(int i = 0; i < group->num_points; i++) {
		printf("(%f   %f)\n", group->points[i].x, group->points[i].y);
	}
}
...
void plot_data(CoordinateSystem *coord_sys, DataArray2 *data) {
	...
	print_data_point_group_points(coord_sys->groups[0]);
