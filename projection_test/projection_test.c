...
gboolean on_scroll(GtkWidget *widget, GdkEventScroll *event, Camera *camera) {
	...
	draw_celestial_system(camera->screen.cr, test_camera, test_system, 0, camera->screen.width, camera->screen.height);
	return TRUE;
}

void on_draw_projection(GtkWidget *widget, cairo_t *cr, gpointer user_data) {
	draw_camera_image(cr, test_camera, TRUE);
}
...
void activate_test(GtkApplication *app, gpointer user_data) {
	...
	draw_celestial_system(test_camera.screen.cr, test_camera, test_system, 0, test_camera.screen.width, test_camera.screen.height);
