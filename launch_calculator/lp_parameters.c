		...
		for(int j = 0; j < pad.steps[1]; j++) {
			...
			for(int k = 0; k < pad.steps[2]; k++) {
				...
				if(lr.pe > min_pe && lr.dv < min) {
					...
					min_params[num_params] = lr.dv;
					min_params[num_params+1] = lr.pe;
					min = lr.dv;
				}
			}
		}
...
void lp_param_fixed_payload_analysis4(struct LV lv, double payload_mass, double *best_params) {
	...
	double min_params[num_params+2];
	min_params[num_params] = 1e9;
	min_params[num_params+1] = 0;
	...
	for(int i = 0; i < 3; i++) {
		...
		for(int j = 0; j < pad.steps[0]; j++) pta_min_params[j] = (double*) malloc((num_params+2) * sizeof(double));

		struct ParamThreadArgs pta = {lv,payload_mass, pta_min_params, pad};
		...
		for(int j = 0; j < pad.steps[0]; j++) {
			if(pta_min_params[j][num_params] < min_dv && pta_min_params[j][num_params+1] > min_pe) {
				for(int k = 0; k < num_params+2; k++) min_params[k] = pta_min_params[j][k];
...
//    struct ParamThreadArgs *thread_args = (struct ParamThreadArgs *)arg;
...
//    struct ParamThreadArgs thread_args[(int)size_b];
//
